/* =========================================================================
   TaxInvoicePrint.js
   -------------------------------------------------------------------------
   Standalone Tax Invoice print / PDF system.

   PUBLIC API
     window.generateTaxInvoicePrint(data)
   ...
   PDF SAVE + STATUS CONFIRM
     On "Save" or "Print / Save as PDF":
       ...
       3. POST the same blob to:
             /api/erp/tax-invoices/<invoiceNumber>/confirm/
       4. Toolbar status text reflects the confirmed state.

   AUTHENTICATION
     HttpOnly refresh cookie + csrftoken cookie (same as PO/Quotation/DC print).
   ========================================================================= */

(function () {
  "use strict";

  var PAYLOAD_KEY = "tip-print-payload-v1";
  var ROOT_ID = "tip-print-app-root";
  ...
  function buildPdfBlob(sourceElement, filename) {
    return loadHtml2Pdf().then(function (html2pdf) {
      ...
      var options = {
        margin: 0,
        filename: filename || "tax-invoice.pdf",
  ...
  function postPdfToBackend(invoiceNumber, pdfBlob, filename) {
    if (!invoiceNumber) {
      return Promise.reject(
        new Error("Cannot confirm: Invoice number is missing.")
      );
    }
    if (!pdfBlob) {
      return Promise.reject(
    ...
    formData.append("pdf", pdfBlob, filename || invoiceNumber + ".pdf");

    var csrfToken = readCookie("csrftoken");
    if (!csrfToken) {
      console.warn(
        "[TaxInvoicePrint] CSRF token was not found in document.cookie."
      );
    }
    ...
    return fetch(url, {
      ...
      .catch(function (networkError) {
        throw new Error(
          "Network error while confirming the Tax Invoice: " +
  ...
  function generateTaxInvoicePrint(data) {
    if (!data) {
      console.error(
        "[TaxInvoicePrint] generateTaxInvoicePrint() called without invoice data."
      );
      return;
    }
    ...
    try {
      ...
      console.error(
        "[TaxInvoicePrint] Could not stage invoice data for the print tab:",
        e
      );
      return;
    }
    var printTab = window.open("/TaxInvoicePrint.html", "_blank");
    if (!printTab) {
      window.alert(
  ...
  if (typeof window !== "undefined") {
    window.generateTaxInvoicePrint = generateTaxInvoicePrint;
  }
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (!payload || !payload.data) {
      root.appendChild(
        el(
          "p",
          "tip-error",
          "No invoice data was found for this print preview. Please go back to the Tax Invoice form and use " +
    ...
    buildDocument(root, payload.data).catch(function (err) {
      console.error("[TaxInvoicePrint] Failed to build invoice:", err);
      root.appendChild(
        el(
          "p",
          "tip-error",
  ...
  function buildHeaderNode(company) {
    ...
    centerCell1.textContent = "INVOICE";
  ...
  function buildMetaTableNode(formData, placeOfSupply) {
    ...
    tbody.appendChild(
      row(
        ...
        placeOfSupply && placeOfSupply.state
          ? placeOfSupply.state +
              " & " +
              (placeOfSupply.stateNameCode || "")
          : placeOfSupply && placeOfSupply.stateNameCode
      )
    );
  ...
  function buildBottomBlockNode(company, formData) {
    ...
    var enclosureLabels = Object.keys(formData.enclosures || {}).filter(
      function (k) {
        return formData.enclosures[k];
      }
    );
    var enclosureHtml = enclosureLabels.length
    ...
    var enclLeftHtml = enclosureLabels.length
  ...
  async function buildDocument(root, data) {
    ...
    var placeOfSupply = data.placeOfSupply || {};
    ...
    var metaTableNode = buildMetaTableNode(formData, placeOfSupply);
    ...
    function handleSaveAndConfirm() {
      ...
      var invoiceNumber = (formData.invoiceNumber || "").trim();

      if (!invoiceNumber) {
        status.textContent =
          "Tax Invoice — Save failed (missing invoice number)";
        window.alert(
          "Cannot save the PDF: Invoice number is missing."
        );
        return;
      }
      ...
      buildPdfBlob(pagesHost, filename)
        .then(function (blob) {
          // 1. Local download — same behaviour as PO/Quotation/DC.
          ...
          return postPdfToBackend(invoiceNumber, blob, filename);
        })
        .then(function () {
          status.textContent =
            "Tax Invoice " +
        ...
        .catch(function (error) {
          console.error(
            "[TaxInvoicePrint] Save / confirm failed:",
            error
          );
          ...
          window.alert(
            "The PDF was downloaded locally, but the Tax Invoice could not be confirmed on the server.\n\n" +
              (error && error.message
                ? error.message
                : "Unknown error.")
          );
